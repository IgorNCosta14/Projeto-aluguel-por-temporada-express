...
class UpdatePropertyController {
    async handle(request: Request, response: Response): Promise<Response> {
        const { propertyName, description, propertyNumber, typeProperty, dailyRate } = request.body
        const { id: propertyOwner }  = request.user;
        const { id } = request.params;

        const updatePropertyUseCase = container.resolve(UpdatePropertyUseCase);

        const property = await updatePropertyUseCase.execute({id, propertyName, description, typeProperty, propertyNumber ,dailyRate, propertyOwner })
